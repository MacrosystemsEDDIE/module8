...
for (i in 1:length(forecast_dates)) {
  ...
  fcast <- as.data.frame(fcast)
    ...
    write.csv(fcast, paste0("data/wq_forecasts/forecast_day", 15-i, ".csv"), row.names = FALSE)
    
    
}

write.csv(data, paste0("data/wq_forecasts/mock_chl_obs.csv"), row.names = FALSE)
